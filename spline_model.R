...
spline_model = function(tt){
  ...
  model1 = brm(y~1+x1+x2+x3+x4+y_pre+trt+s(time, by=trt, bs="cr", k=5)+ (1|id), family = gaussian(), 
...
for (i in 1:10) {
  ...
  spline1_1[[i]] <-
    future_map(as.list((1 + (i - 1) * 25):(25 + (i - 1) * 25)), spline_model)
}
